...
class Attention(nn.Module):
    def __init__(self, dim, heads = 8, dim_head = 64, dropout = 0.):
        ...
        self.scale = dim_head ** -0.5 # divisor for attention
        ...
        self.kv_linear = nn.Linear(dim, inner_dim*2, bias=False)
        #self.v_linear = nn.Linear(dim, dim_head, bias=False)
        # and the output linear layer followed by dropout
        self.out = nn.Sequential(nn.Linear(inner_dim, dim), nn.Dropout(dropout))
    ...
    def forward(self, x, context = None, kv_include_self = False):
        ...
        q = rearrange(q, 'b n (h d) -> b h n d', h=h) # rearranging to attend the heads individually
        ...
        out = rearrange(out, 'b h n d -> b n (h d)') # concatenation of the heads
        out = self.out(out)
...
class CrossTransformer(nn.Module):
    ...
    def __init__(self, sm_dim, lg_dim, depth, heads, dim_head, dropout):
        ...
        for _ in range(depth):
            ...
            sm_to_lg = ProjectInOut(sm_dim, lg_dim, PreNorm(lg_dim, Attention(lg_dim, heads = heads, dim_head = dim_head, dropout = dropout)))
            # large CLS attends to small patches
            lg_to_sm = ProjectInOut(lg_dim, sm_dim, Attention(sm_dim, heads = heads, dim_head = dim_head, dropout = dropout))
    ...
    def forward(self, sm_tokens, lg_tokens):
        ...
        for sm_to_lg, lg_to_sm in self.layers:
            sm_cls = sm_to_lg(sm_cls, context=lg_patch_tokens, kv_include_self=True) + sm_cls
            lg_cls = lg_to_sm(lg_cls, context=sm_patch_tokens, kv_include_self=True) + lg_cls
...
class MultiScaleEncoder(nn.Module):
    def __init__(
        ...
        *,
        ...
        self.layers = nn.ModuleList([])
        for _ in range(depth):
            sm_enc = Transformer(sm_dim, **sm_enc_params, dropout=dropout)
            lg_enc = Transformer(lg_dim, **lg_enc_params, dropout=dropout)
            cross = CrossTransformer(sm_dim=sm_dim, lg_dim=lg_dim, depth=cross_attn_depth, heads=cross_attn_heads, dim_head=cross_attn_dim_head, dropout=dropout)
            self.layers.append(nn.ModuleList([
...
class CrossViT(nn.Module):
    def __init__(
        ...
        *,
        ...
        self.sm_embedding = ImageEmbedder(dim=sm_dim, image_size = image_size, patch_size=sm_patch_size, dropout=emb_dropout)
        self.lg_embedding = ImageEmbedder(dim=lg_dim, image_size = image_size, patch_size=lg_patch_size, dropout=emb_dropout)
    ...
    def forward(self, img):
        ...
        sm_tokens = self.sm_embedding(img)
        lg_tokens = self.lg_embedding(img)
